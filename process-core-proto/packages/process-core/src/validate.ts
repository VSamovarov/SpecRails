...
export function validateSpecV1(spec: ProcessSpecV1, opts: ValidateOptions = {}): ValidationResult {
  ...
  for (const [stepId, s] of Object.entries(spec.steps)) {
    if (!s.label?.trim())
      issues.push(issue("error", "label_empty", "step.label is required", `steps.${stepId}.label`, stepId))
    if (!s.lane?.trim())
      issues.push(issue("error", "lane_empty", "step.lane is required", `steps.${stepId}.lane`, stepId))
    ...
    if (hasNext && hasBranches) {
      issues.push(
        issue("error", "next_and_branches", "step cannot have both 'next' and 'branches'", `steps.${stepId}`, stepId)
      )
    }
    ...
    if (hasBranches) {
      s.branches!.forEach((b, idx) => {
        if (!b.when?.trim()) {
          issues.push(
            issue(
              "error",
              "branch_when_empty",
              "branch.when is required",
              `steps.${stepId}.branches[${idx}].when`,
              stepId
            )
          )
        }
        if (!b.to?.trim() || !stepIds.has(b.to)) {
          issues.push(
            issue("error", "branch_to_unknown", `Unknown step '${b.to}'`, `steps.${stepId}.branches[${idx}].to`, stepId)
          )
        }
      })
      ...
      if (dup)
        issues.push(
          issue(
            "warn",
            "branch_when_duplicate",
            `Duplicated branch condition '${dup}'`,
            `steps.${stepId}.branches`,
            stepId
          )
        )
    }
  ...
  for (const id of stepIds) {
    if (!reachable.has(id))
      issues.push(issue("warn", "unreachable", `Step '${id}' is unreachable from entry`, `steps.${id}`, id))
  }
